...
static void generate_proposals(std::vector<ArmorObject> &output_objs, std::vector<float> &scores,
...
static void nms_merge_sorted_bboxes(std::vector<ArmorObject> &faceobjects, std::vector<int> &indices,
                                    float nms_threshold)
{
    indices.clear();
...
OpenVINODetector::OpenVINODetector(const std::filesystem::path &model_path, const std::string &device_name,
                                   float conf_threshold, int top_k, float nms_threshold, bool auto_init)
    : model_path_(model_path), device_name_(device_name), conf_threshold_(conf_threshold), top_k_(top_k),
...
void OpenVINODetector::init()
{
    ...
    compiled_model_ = std::make_unique<ov::CompiledModel>(
        ov_core_->compile_model(model, device_name_, ov::hint::performance_mode(ov::hint::PerformanceMode::LATENCY)));
...
std::future<bool> OpenVINODetector::push_input(const cv::Mat &rgb_img, int64_t timestamp_nanosec)
{
    ...
    // Start async detect
    return std::async(std::launch::async, &OpenVINODetector::process_callback, this, resized_img, transform_matrix,
                                        ...
                                        int64_t timestamp_nanosec, const cv::Mat &src_img)
{
    ...
    cv::Mat blob = cv::dnn::blobFromImage(resized_img, 1., cv::Size(INPUT_W, INPUT_H), cv::Scalar(0, 0, 0), true);

    // Feed blob into input
    auto input_port = compiled_model_->input();
    ov::Tensor input_tensor(input_port.get_element_type(), ov::Shape(std::vector<size_t>{1, 3, INPUT_W, INPUT_H}),
                            blob.ptr(0));
    ...
    // Parse YOLO output
    generate_proposals(objs_tmp, scores, rects, output_buffer, transform_matrix, this->conf_threshold_,
                       this->grid_strides_);
...
void OpenVINODetector::display(cv::Mat &image2show, ArmorObject object)
{
    // 绘制十字瞄准线
    line(image2show, cv::Point2f(image2show.size().width / 2, 0),
         cv::Point2f(image2show.size().width / 2, image2show.size().height), {0, 255, 0}, 1);
    line(image2show, cv::Point2f(0, image2show.size().height / 2),
         cv::Point2f(image2show.size().width, image2show.size().height / 2), {0, 255, 0}, 1);
    ...
    circle(image2show, cv::Point(object.pts[0].x, object.pts[0].y), 3, cv::Scalar(255, 0, 0), 5);
    circle(image2show, cv::Point(object.pts[1].x, object.pts[1].y), 3, cv::Scalar(0, 255, 0), 5);
    circle(image2show, cv::Point(object.pts[2].x, object.pts[2].y), 3, cv::Scalar(0, 0, 255), 5);
    circle(image2show, cv::Point(object.pts[3].x, object.pts[3].y), 3, cv::Scalar(255, 255, 0), 5);
    circle(image2show, cv::Point((object.pts[0].x + object.pts[2].x) / 2, (object.pts[0].y + object.pts[2].y) / 2), 5,
           cv::Scalar(255, 255, 255), 5);

    // 绘制装甲板四点矩形
    for (int i = 0; i < 4; i++)
    {
        line(image2show, object.pts[i], object.pts[(i + 1) % 4], cv::Scalar(100, 200, 0), 3);
    }

    int box_top_x = object.pts[0].x;
    int box_top_y = object.pts[0].y;
    switch (object.color)
    {
    case ArmorColor::BLUE:
        cv::putText(image2show, "Blue_" + std::to_string(static_cast<int>(object.number)),
                    cv::Point(box_top_x + 2, box_top_y), cv::FONT_HERSHEY_TRIPLEX, 1, cv::Scalar(255, 0, 0));
        break;
    case ArmorColor::RED:
        cv::putText(image2show, "Red_" + std::to_string(static_cast<int>(object.number)),
                    cv::Point(box_top_x + 2, box_top_y), cv::FONT_HERSHEY_TRIPLEX, 1, cv::Scalar(0, 0, 255));
        break;
    case ArmorColor::NONE:
        cv::putText(image2show, "None_" + std::to_string(static_cast<int>(object.number)),
                    cv::Point(box_top_x + 2, box_top_y), cv::FONT_HERSHEY_TRIPLEX, 1, cv::Scalar(0, 255, 0));
        break;
    case ArmorColor::PURPLE:
        cv::putText(image2show, "Purple_" + std::to_string(static_cast<int>(object.number)),
                    cv::Point(box_top_x + 2, box_top_y), cv::FONT_HERSHEY_TRIPLEX, 1, cv::Scalar(255, 0, 255));
        break;
    default:
        break;
    }
...
int OpenVINODetector::getArmorType(ArmorObject object)
{
...
{
    float distance =
        sqrt(pow((armor_center.x - last_armor_center.x), 2) + pow((armor_center.y - last_armor_center.y), 2));
    return distance;
}

void OpenVINODetector::getOptimalTarget(std::vector<ArmorObject> objects, ArmorObject &optimal_object,
                                        ArmorColor desired_color)
{
