...
{
  public:
    using DetectorCallback = std::function<void(const std::vector<ArmorObject> &, int64_t, const cv::Mat &)>;
  ...
  public:
     ...
     */
    explicit OpenVINODetector(const std::filesystem::path &model_path, const std::string &device_name,
    ...
    /**
     * @brief show the inference result
     *
     * @param image2show
     * @param object
     */
    void display(cv::Mat &image2show, ArmorObject object);
  ...
  private:
    bool process_callback(const cv::Mat resized_img, Eigen::Matrix3f transform_matrix, int64_t timestamp_nanosec,
