...
VideoCapture::VideoCapture(const CameraParam &_camera_param)
{
    ...
    {
        cameraInit(_camera_param.resolution.cols, _camera_param.resolution.rows, _camera_param.camera_exposuretime);
...
int VideoCapture::cameraInit(const int _CAMERA_RESOLUTION_COLS, const int _CAMERA_RESOLUTION_ROWS,
                             const int _CAMERA_EXPOSURETIME)
{
    ...
    g_pRgbBuffer = static_cast<unsigned char *>(
        malloc(tCapability.sResolutionRange.iHeightMax * tCapability.sResolutionRange.iWidthMax * 3));
...
{
    // After successfully calling CameraGetImageBuffer, CameraReleaseImageBuffer must be called to release the acquired
    // buffer Otherwise, when CameraGetImageBuffer is called again, the program will hang and block until
    // CameraReleaseImageBuffer is called in another thread
    if (iscamera0_open)
    {
        CameraReleaseImageBuffer(hCamera, pbyBuffer);
    }
